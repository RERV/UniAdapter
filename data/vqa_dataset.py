...
class vqa_dataset(Dataset):
    ...
    def __getitem__(self, index):    
        ...
        if self.split == 'test':
            image_path = os.path.join('/home/haoyu_lu/image_dataset/coco2014', ann['image'])    
        else:
            if ann['dataset']=='vqa':
                image_path = os.path.join(self.vqa_root,ann['image'].split('/')[-1] if self.split != 'test' else ann['image'])    
            elif ann['dataset']=='vg':
